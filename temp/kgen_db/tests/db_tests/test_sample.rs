...
pub fn mutate_row_data(row_data: &mut SampleRowData) {
...
pub async fn select_and_compare_assert<T>(pool_conn: &T, values: &Vec<SampleRowData>, label: &str)
where
    T: Deref<Target = Client>,
{
    let selected = TableSample::select_all(&pool_conn, 4).await;
...
#[inline]
pub fn get_sample_rows_sorted(rows: &[SampleRowData]) -> BTreeSet<String> {
...
pub fn get_sample_rows() -> Vec<SampleRowData> {
    vec![
        SampleRowData {
            the_name: "a".into(),
            the_small_int: -32768,
...
/// Test by delete, bulk insert, select, bulk upsert, then delete for sample
...
pub async fn test_crud() {
    ...
    let deleted = TableSample::delete_all(&conn).await.unwrap();
    ...
    */
    {
        assert_eq!(0, TableSample::select_all(&conn, 4).await.len());
    }
    ...
    */
    {
        let inserted = TableSample::basic_insert(&conn, &samples).await.unwrap();
    ...
    */
    {
        let inserted = TableSample::bulk_insert(&conn, &samples, 4).await.unwrap();
    ...
    */
    {
        ...
        let upserted = TableSample::bulk_upsert(&conn, &samples, 4).await.unwrap();
    ...
    */
    {
        let deleted = TableSample::delete_all(&conn).await.unwrap();
        tracing::info!("Deleted all {deleted} TableSample entries");
        assert_eq!(deleted as usize, samples.len());
        let selected = TableSample::select_all(&conn, 4).await;
        assert_eq!(0, selected.len());
    }
}

// α <mod-def test_sample>
// ω <mod-def test_sample>
